...
│
├── data/                      # Данные приложения (создаётся при работе)
│   ├── user+79001234567.session  # Telethon сессия (имя включает номер телефона)
...
## Папка данных `data/` (создаётся при работе)

Все данные приложения хранятся в папке `data/`:

| Файл | Описание |
|------|----------|
| `data/config.json` | Конфигурация: выбранные каналы, настройки вебхука |
| `data/data.db` | SQLite база данных с сообщениями и отправителями |
| `data/user{phone}.session` | Файл сессии Telethon (имя включает номер телефона, например `user+79001234567.session`) |

Папка `data/` добавлена в `.gitignore` и не отслеживается в репозитории.
